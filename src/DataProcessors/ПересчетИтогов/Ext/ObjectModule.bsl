...
// Возвращает имена очищаемых таблиц итогов регистров.
//
// Параметры:
//  ИменаРегистров	 - Массив - Имена регистров.
// 
// Возвращаемое значение:
//  Массив - Массив имен таблиц регистров.
//
...
// Возвращает доступность использования MS-SQL.
// Доступность определяется по типу платформы.
//
// Возвращаемое значение:
//  Булево - Данные соединения ИБ.
//
...
// Возвращает данные соединения информационной базы.
//
// Возвращаемое значение:
//  Структура - Данные соединения ИБ.
//
Функция ПолучитьСтруктуруСоединенияИнформационнойБазы() Экспорт
	...
	Если ИнформационнаяБазаФайловая() Тогда
		Возврат Сред(СтрокаСоединенияСБД, ДлинаПрефикса, СтрДлина(СтрокаСоединенияСБД) - ДлинаПрефикса);
	КонецЕсли;
	...
	НачальнаяПозицияКопирования = ДлинаПрефикса + 1;
	...
	НачальнаяПозицияКопирования = ДлинаПрефикса;
	ПозицияТочкиСЗапятой = СтрНайти(СтрокаСоединенияСБД, ";");
	КонечнаяПозицияКопирования = ПозицияТочкиСЗапятой - 2; 
...
// Формирует строку подключения к MS-SQL.
//
// Параметры:
//  ПараметрыПодключения - Структура - См. ПараметрыПодключения().
// 
// Возвращаемое значение:
...
// Пересчитывает методами платформы итоги регистров.
//
// Параметры:
//  ИменаРегистров	 - Массив - Имена регистров.
// 
...
Функция ИменаТаблицХраненияИтогов(ПолноеИмяРегистра)
	...
	Для каждого СтрокаТаблицы Из ТаблицаСтруктуры Цикл
		Если ЭтоОчищаемаяТаблица(СтрокаТаблицы.Назначение) Тогда
...
Функция ЭтоОчищаемаяТаблица(Назначение)

	Назначения = Новый Массив;
	
	Назначения.Добавить("Итоги");
	Назначения.Добавить("Обороты");
	// РС
	Назначения.Добавить("ИтогиСрезПоследних");
	Назначения.Добавить("ИтогиСрезПервых");
	// регисты бухгалтерии
	Назначения.Добавить("ИтогиМеждуСчетами");
	Назначения.Добавить("ИтогиПоСчетам");
	Назначения.Добавить("ИтогиПоСчетамССубконто1");
	Назначения.Добавить("ИтогиПоСчетамССубконто2");
	Назначения.Добавить("ИтогиПоСчетамССубконто3");
	
	Возврат (НЕ Назначения.Найти(Назначения) = Неопределено);
...
//
Процедура ОчиститьТаблицыСредствамиСУБД(ПараметрыПодключения, Connection, ИменаОчищаемыхТаблиц)
...
Процедура ОчиститьТаблицыРегистров(ИменаОчищаемыхТаблиц)
	...
	ОчиститьТаблицыСредствамиСУБД(ПараметрыПодключения, Connection, ИменаОчищаемыхТаблиц);
