...
&НаКлиенте
Процедура ОткрытьСтраницуРазработкиНаИнфостарте(Команда)
	
	Попытка
		ЗапуститьПриложение("http://www.infostart.ru/public/" + НомерПубликации());
	Исключение
	КонецПопытки;
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ДоступнаРаботаСMSSQL Тогда
		СообщитьИнформацию("Обработка предназначена для работы в клиент-серверном варианте с СУБД MS-SQL. Очистка таблиц итогов средствами СУБД будет недоступна.");	
	КонецЕсли;
...
Процедура ДеревоДанныхПометкаПриИзменении(Элемент)
	...
	Если Элементы.ДеревоДанных.ТекущиеДанные.Пометка Тогда
		РекурсивноВверхСнятьУстановитьПометки(Элементы.ДеревоДанных.ТекущиеДанные, Элементы.ДеревоДанных.ТекущиеДанные.Пометка);
	КонецЕсли;
...
КонецФункции // ПолучитьСтрокуПодключенияSQL()
...
КонецФункции // ВыполнитьПодключениеКСерверуSQLНаСервере()
...
КонецФункции // ВыполнитьПодключениеКСерверуSQLНаКлиенте()
...
Функция ПолучитьСтруктуруСоединенияИнформационнойБазы()
	...
	ИмяБД = Сред(СтрокаСоединенияСБД, НачальнаяПозицияКопирования, КонечнаяПозицияКопирования - НачальнаяПозицияКопирования + 1);
