...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ДоступнаРаботаСMSSQL Тогда
		ТекстСообщения = "Обработка предназначена для работы в клиент-серверном варианте с СУБД MS-SQL.
...
Процедура КомандаВыполнить(Команда)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Начало обработки. " + ОбщегоНазначенияКлиент.ДатаСеанса());
	
	ВыполнитьОбработку();
	
	ТекстСообщения = СтрШаблон("Выполнение обработки завершено. %1", ОбщегоНазначенияКлиент.ДатаСеанса());
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОчиститьТаблицыРегистров(ПараметрыПодлючения, ИменаОчищаемыхТаблиц)
	...
	ОчиститьТаблицыСредствамиСУБД(Connection, ПараметрыПодлючения, ИменаОчищаемыхТаблиц);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОчиститьТаблицыСредствамиСУБД(Connection, ПараметрыПодлючения, ИменаОчищаемыхТаблиц)
